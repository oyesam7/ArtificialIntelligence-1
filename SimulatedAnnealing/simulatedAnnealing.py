import math
# 1.start point P: start random or defined value:
def SA(P, Tmax, Ftarget, dT):
    T = Tmax  # 2.set temperature to Tmax
    while (true):# 3.objective function F evaluate samples
        if P.F(P) >= Ftarget: return P  # 4.return P as solution
        neighbors = P.Gn(P)# 5.generate n neighbors of P
        for i in range(len(neighbors)): eneighbors[i] = P.F(neighbors[i])  # 6. Evaluate each neighbor
        ...
        if x > p:   P = Pmax  # 11.
        else: P = random.choice(eneighbors)  # a random choice among n neighbors
        T = T - dT  # 13.
